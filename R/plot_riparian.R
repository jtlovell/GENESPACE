...
#' plotted first
#' @param ylabBuff numeric giving the absolute size buffer surrounding the
#' chromosome labels.
#'
...
plot_riparian <- function(gsParam,
                          ...
                          palette = gs_colors,
                          chrLabCex = 1,
                          ylabBuff = ((0.01 * (length(genomeIDs) / 8)) + 0.02) / scalePlotHeight,
                          braidBorderLwd = 0,
  ...
  if(is.null(highlightBed)){
    ...
    pltDat <- riparian_engine(
      ...
      ylabBuff = ylabBuff, scaleGapSize = scaleGapSize, palette = palette,
...
riparian_engine <- function(blk,
 ...
 blkID <- refChr <- genome1 <- genome2 <- genome <- ordByFun <- chr <-
  ...
  sbData <- make_scaleBarData(
    chrLengths = clens, ylabBuff = ylabBuff*1.5, useOrder = useOrder)
  ...
  p <- p +
    ...
    geom_segment(
      ...
      lwd = chrLabBorderLwd/2, col = chrFill)+
    geom_text(
