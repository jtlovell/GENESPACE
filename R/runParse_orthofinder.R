...
runParse_orthofinder<-function(peptide.dir,
  ...
  sm$map.rank = as.numeric(factor(sm$genome1, levels = names(ploidy)))
  ...
  bl = lapply(sl, function(i){
    ...
    d = merge(si1,merge(si2, d, by = "gn2"),by = "gn1")
    d$gn1<-NULL
    d$gn2<-NULL
    d = merge(gff1,merge(gff2, d, by = "id2"),by = "id1")
    d = d[d$og1 == d$og2,]
  ...
  if(verbose)
    cat("Writing culled blast output to",results.dir,"\n")
  if(!file.exists(results.dir))
    system(paste("mkdir",results.dir))
  write.csv(out, file = file.path(results.dir,"culledblasts.csv"), row.names = F)
