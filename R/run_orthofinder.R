...
#'
#' @param peptide.dir The path to the directory containing
#' the peptide fasta sequence files. If NULL, the orthofinder algorithm is
#' run on pre-existing blast results.
#' @param blast.dir The path to the directory where the blast
#' results should be stored
#' @param tmp.dir The path to the directory where temporary
#' files will be stored then deleted
#' @param blast.threads Numeric, how many parallel threads
#' should be used for blast?
#' @param og.threads Numeric, how many parallel threads
#' should be used for orthogroup construction
#' @param og.silent Logical, should orthofinder stout be suppressed?
#' @param remake.pep.only Logical, should the process be stopped once peptide
#' files are re-formatted?
...
run_orthofinder <- function(peptide.dir = NULL,
  ...
  if (remake.pep.only) {
    ...
    if (verbose)
      cat("Prepping orthofinder-formatted files\n")

    system(paste("orthofinder",
                 "-f", tmp.dir,
                 "-t", blast.threads,
                 "-a", og.threads,
                 "-S diamond",
