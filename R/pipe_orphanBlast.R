...
pipe_orphanBlast <- function(dir.list,
  ...
  ########################################################
  ########################################################
  split_pepByOrphan = function(orphan.md, pep.fastas, n.cores){
    ...
    blk.md <- rbindlist(lapply(names(pep.list), function(i){
      if(verbose)
        if(which(names(pep.list)==i) %% nb == 0)
          cat(which(names(pep.list)==i),"/",length(pep.list),"\n\t")
      ...
      ass.file = file.path(assembly.dir,paste0(genome.list[[i]],".fa"))
  ...
  ########################################################
  ########################################################
  pipe_diamondBlastx <- function(blast.inputs, verbose = T){
    n = nrow(blast.inputs)
    nb = ifelse(n<100,5,
                ifelse(n<1000,50,
  ...
  ########################################################
  ########################################################
  read_blastx = function(blast.files, n.cores = 1){
